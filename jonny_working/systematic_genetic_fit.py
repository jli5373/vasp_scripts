...
run_fits = True                    #Change to True if you would like to do new fits. Leave False if you only want to generate new plots

#fit parameters
A = np.linspace(.1,2,3)
B = np.linspace(0,2,3)
kt = np.linspace(.01,.2,5)
...
for i in range(len(A)):
    for j in range(len(B)):
        for k in range(len(kt)):
            ...
            os.system('python ~/Desktop/vasp_scripts/jonny_working/plot_clex_hull_data.py `pwd` 0')
            os.system('python ~/Desktop/vasp_scripts/jonny_working/plot_clex_hull_data.py `pwd` 1')
            os.chdir('../')
